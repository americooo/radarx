...
function ResultsView() {
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold tracking-tight text-emerald-400">Results</h1>
            <p className="mt-1 text-sm text-slate-400">
                Latest snapshot per target: subdomains, ports, TLS info.
            </p>
            ...
            {!loading && snapshot && (
                <>
                    <div className="mt-4 text-sm text-slate-500">
                        Taken: {new Date(snapshot.taken_at).toLocaleString()} · {assets.length} of {snapshot.assets.length} asset
                        {snapshot.assets.length === 1 ? '' : 's'} shown
                    </div>
